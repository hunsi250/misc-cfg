...
class HotManga extends ComicSource {
    ...
    search = {
        load: async (keyword, options, page) => {
            let author;
            // 统一语法: 支持 artist:"xxx" 前缀 (与原生"作者:"前缀等价)
            const _p = this.parseSearchSyntax(keyword);
            if (_p.fields.artist && _p.fields.artist.length) {
                author = _p.fields.artist.join(" ");
            } else if (keyword.startsWith("作者:")) {
                author = keyword.substring("作者:".length).trim();
            }
            if (author && !(author in this.author_path_word_dict)) {
                // 未命中作者映射表: 退化为用作者名做普通搜索
                keyword = author;
                author = null;
            }
            let res;

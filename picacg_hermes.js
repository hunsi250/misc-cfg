class Picacg extends ComicSource {
    ...
    url = "https://uhxvvf0u.de5.net/update/picacg_hermes.js"
    ...
    search = {
        load: async (keyword, options, page) => {
            // 统一语法转换: artist:"xxx" / parody:"xxx" 等前缀 → 纯关键词
            const _p = this.parseSearchSyntax(keyword);
            let _kw = [];
            for (const _v of Object.values(_p.fields)) _kw.push(..._v);
            if (_p.plain) _kw.push(_p.plain);
            keyword = _kw.join(" ") || keyword;
            let res = await Network.post(
